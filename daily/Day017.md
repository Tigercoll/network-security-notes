...
- 掌握 PowerShell 的对象管道、条件判断、循环、函数、错误处理
- 能独立完成日志批量处理、数据统计、导出等自动化任务
...
**目标**：掌握 ForEach-Object、随机数、日期格式化
...
**目标**：掌握管道、Group-Object、Format-Table
...
### 任务 4：导出所有“fail”操作的日志到 fail.csv
...
1. 用 PowerShell 写一个脚本，统计每小时的登录次数（按时间分组）
2. 写一个脚本，找出 1 小时内操作次数最多的用户
3. 用正则提取所有“fail”操作的时间和用户名
4. 写一个函数，批量处理多个日志文件，统计总的“fail”次数
5. 用 try/catch 包裹文件读取，遇到不存在的文件输出“文件缺失”

### 评估标准

- ✅ 能用管道和 Group-Object 做分组统计
- ✅ 能用 Export-Csv 导出结果
- ✅ 能写出带参数的函数
- ✅ 能用 try/catch 做错误处理
- ✅ 能用正则提取日志字段
