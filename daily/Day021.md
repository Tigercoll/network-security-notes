...
├─ Day015：Linux 安全基础
│   ├─ 权限管理（rwx/SUID/SGID）
│   ├─ 用户与组（useradd/sudo/sudoers）
│   ├─ 服务管理（systemd/监听端口）
│   ├─ 日志审计（journalctl/auth.log）
│   └─ 防火墙（ufw/iptables/firewalld）
│
├─ Day016：Windows 安全基础
│   ├─ UAC 原理（4个安全级别）
│   ├─ 用户与组（管理员组审计）
│   ├─ 事件日志（4624/4625/4672/4688）
│   ├─ 防火墙（入站规则审计）
│   └─ 本地安全策略（密码/锁定策略）
│
├─ Day017：PowerShell 脚本
│   ├─ 对象管道与 Select-Object/Where-Object
│   ├─ 变量、循环、条件、函数
...
└─ Day020：服务枚举与指纹识别
    ├─ HTTP 枚举（curl/浏览器）
    ├─ SSH Banner 获取（nc/telnet）
    ├─ 数据库枚举（mysql-info）
    ├─ RDP/SMB 探测（Nmap NSE）
    └─ 结果解析（Python/PowerShell）
```
...
#### 需求
1. 读取 `demo.log`（格式：`时间, 用户, 操作`）
2. 统计每个用户的操作次数
3. 导出为 CSV
4. 用 try/except 处理文件不存在的情况
...
def analyze_log(log_file, output_file):
    try:
        ...
        print(f"✅ 已导出 {len(stats)} 个用户的统计结果到 {output_file}")
...
# 使用
analyze_log("demo.log", "user_stats.csv")
```
...
- ✅ 完成 Windows 或 Linux 主机自查（至少一个）
- ✅ 完成授权目标的端口扫描+服务枚举
- ✅ 写出至少 1 个可复用的脚本（PowerShell 或 Python）
- ✅ 输出一份完整的第 3 周周报
- ✅ 识别并整改至少 3 个安全风险
